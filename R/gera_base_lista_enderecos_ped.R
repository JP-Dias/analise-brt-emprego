...
arquivos_gp_xls <- list.files(path = "Dados/BasesGP", 
                              pattern = "\\.xls$|\\.xlsx$", 
                              full.names = TRUE)

arquivos_gp_sav <- list.files(path = "Dados/BasesGP", 
                              pattern = "\\.sav$", 
                              full.names = TRUE)[c(1, 3, 13, 14, 39)]

# Função para ler e empilhar
...
# Salva base
saveRDS(bind_GP,"Rds/base_enderecos_ped.RDS")
