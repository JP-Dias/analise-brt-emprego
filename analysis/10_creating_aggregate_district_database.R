...
dados2 <- dados2 %>%
  mutate(fam_3plus = ifelse(pessoas >= 3, 1, 0))
...
dados_agg <- dados2 %>%
  ...
  summarise(
    ...
    fam_3plus = sum(fam_3plus, na.rm = TRUE),
    n_trabalhadores = n(),
  ...
  mutate(
    across(
      c(informal, trab_plano, mora_mesma_ra, analf, educ_prim, educ_sec, educ_terc,
        preta_parda, setor_serv_comerc, setor_setor_ind, fem, fam_3plus),
      ~ .x / n_trabalhadores,
      .names = "prop_{.col}"
    )
  )
...
saveRDS(dados_agg, file = "dados_agg_reg_completa.rds")
