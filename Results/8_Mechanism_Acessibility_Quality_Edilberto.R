...
vars_needed <- c("trab_plano", "informal", "BRT_Effect", "idade", 
          "fem", "cor", "escol", "pessoas", "setor_atv",
          "reg", "aamm", "conglom")

dados_same <- na.omit(dados3[, vars_needed])

reg1 <- feols(trab_plano ~ BRT_Effect | reg + aamm, ~conglom, data=dados_same)
summary(reg1)

reg2 <- feols(trab_plano ~ BRT_Effect + idade | reg + aamm + fem + cor + escol + pessoas + setor_atv, ~conglom, data=dados_same)
summary(reg2)

reg3 <- feols(informal ~ BRT_Effect | reg + aamm, ~conglom, data=dados_same)
summary(reg3)

reg4 <- feols(informal ~ BRT_Effect + idade | reg + aamm + fem + cor + escol + pessoas + setor_atv, ~conglom, data=dados_same)
summary(reg4)
