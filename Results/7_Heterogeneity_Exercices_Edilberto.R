...
vars_needed <- c(
  "rend_bruto", "horas_trabM",
  "dummy_preto", "BRT_Effect", "idade", "fem", "cor", "escol",
  "pessoas", "setor_atv", "dummy_low_skill", "reg", "aamm", "conglom"
)

dados_same <- dados3[complete.cases(dados3[, vars_needed]), ]
nrow(dados_same)

reg1 <- feols(log(rend_bruto/horas_trabM) ~ dummy_preto*BRT_Effect | reg + aamm, ~conglom, data=dados_same)
summary(reg1)

reg2 <- feols(log(rend_bruto/horas_trabM) ~ BRT_Effect + idade + dummy_preto:BRT_Effect | reg + aamm + fem + cor + escol + pessoas + setor_atv, ~conglom, data=dados_same)
summary(reg2)

reg3 <- feols(log(rend_bruto/horas_trabM) ~ fem*BRT_Effect | reg + aamm, ~conglom, data=dados_same)
summary(reg3)

reg4 <- feols(log(rend_bruto/horas_trabM) ~ BRT_Effect + idade + fem:BRT_Effect | reg + aamm + fem + cor + escol + pessoas + setor_atv, ~conglom, data=dados_same)
summary(reg4)

reg5 <- feols(log(rend_bruto/horas_trabM) ~ dummy_low_skill*BRT_Effect | reg + aamm, ~conglom, data=dados_same)
summary(reg5)

reg6 <- feols(log(rend_bruto/horas_trabM) ~ BRT_Effect + idade + dummy_low_skill:BRT_Effect | reg + aamm + fem + cor + escol + pessoas + setor_atv, ~conglom, data=dados_same)
summary(reg6)
